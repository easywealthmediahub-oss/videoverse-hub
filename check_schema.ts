...
async function checkSchema() {
  console.log('Checking if ad_units table exists...');
  ...
  if (error) {
    console.log('Error querying ad_units table:', error.message);
    
    // If table doesn't exist, try to create it manually through Supabase
    console.log('ad_units table does not exist. Please run the migration manually in your Supabase dashboard.');
...
CREATE TRIGGER handle_ad_units_updated_at
  ...
  FOR EACH ROW EXECUTE FUNCTION public.handle_updated_at();
    ...
    console.log('ad_units table exists and is accessible');
  }
}
